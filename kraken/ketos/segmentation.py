...
def segtest(ctx, **kwargs):
    ...
    # class_pixel_accuracy and class_iu are indexed by position within
    # pixel_idxs (a subset of all classes), not by raw class index.
    for pos, idx in enumerate(pixel_idxs):
        cat, class_name = idx_to_name[idx]
        count = f'{data_module.test_set.dataset.class_stats[cat][class_name]}' if cat != 'aux' else 'N/A'
        table.add_row(cat, class_name, f'{class_pixel_accuracy[pos]:.3f}', f'{class_iu[pos]:.3f}', count)
    console.print(table)
